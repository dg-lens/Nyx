...
function runDependencyAudit(cwd: string, report: FindingsReport): void {
  ...
  if (existsSync(resolve(cwd, 'pnpm-lock.yaml'))) tools.push({ cmd: 'pnpm', args: ['audit', '--json'] });
  else if (existsSync(resolve(cwd, 'yarn.lock'))) tools.push({ cmd: 'yarn', args: ['npm', 'audit', '--json'] });
  else tools.push({ cmd: 'npm', args: ['audit', '--json'] });
  ...
  for (const tool of tools) {
    ...
    try {
      ...
      for (const [pkg, v] of Object.entries(vulns)) {
        ...
        addFinding(report, {
          id: `dep-${pkg}`,
          severity: (['critical', 'high', 'medium', 'low'].includes(sev) ? sev : 'medium') as Finding['severity'],
          category: 'dependency',
...
function scanCSPHeaders(cwd: string, report: FindingsReport): void {
  ...
  if (!foundCSP && existsSync(resolve(cwd, 'next.config.js')) || existsSync(resolve(cwd, 'next.config.mjs'))) {
    addFinding(report, {
      id: 'csp-missing',
      severity: 'medium',
      category: 'headers',
