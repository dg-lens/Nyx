...
Review verbs: `accept` → force-merge the held task(s) into the integration branch
+ CONTINUE (next phase, or shipping if last) — the non-destructive "I looked,
they're fine" override; nothing is re-coded (`forceMergeHeld`; a branch that truly
conflicts re-parks at the gate). `proceed` → deliver+done, shipping only what
already merged (stops the build). `fix` → executing from phase 0 (fresh corrective
wave, threads the directive via `fix_directive`, resets counters). `rollback` →
planning. `abort` → cleanup+aborted.
...
| `redux.ts` | Stage ⑥ (composer stages 1–3 promoted), PHASE-SCOPED to `current_phase`'s tasks. `harvestFacts` → `judgeP1` (deterministic conflict/failed override) → `judgeP2` cross-worktree interface graph (final arbiter; remediation + catastrophic) → `decideMerges` (pure — **interface gate**: merges only if P2-fits ∧ P1 is not a hard failure [`clean` or `semantic_suspect` — P2, the final arbiter, clears the soft `semantic_suspect` verdict; `conflict`/`failed`/`scope_drift`/`signature_drift` stay blocking] ∧ every `deps`/`consumes` producer also merges, fixpoint) → `runMergeQueue` (authoritative). Persists `redux_findings`/`remediation_plan` for the phase. Judges injected; git real (temp-repo tests). |
| `install-deps.ts` | The pipeline's dependency-install step (pipeline runs bypass `preflight.ts`; the smoke verifier is read-only). `detectInstallCommands` (pure) → install commands by ecosystem: node (pnpm/yarn/npm by lockfile) + `npx playwright install chromium` when `@playwright/test` is a dep + `uv sync` for pyproject. `installDeps(base)` runs them best-effort (a failed install → red smoke, not a throw); exec seam injected for tests. |
| `shipping.ts` | Final smoke only (per-phase recovery now lives in executing). `runShipping` runs `defaultSmoke` once → green / review. **`defaultSmoke` is HERMETIC**: pristine `checkout/reset/clean`, then **`installDeps(base)` BEFORE the verifier** (so the gate sees real `node_modules`; `git clean -fd` keeps gitignored deps across rounds → `pipeline.deps.installed`), verify-only prompt + post-run `git status` dirty-guard (`interpretSmoke`) — a verifier that mutates the tree fails the run. **`runDiagnosticRound`** (exported) re-implements held tasks from a clean integration baseline + only ADOPTS an in-scope fix (`classifyDiagnosticFix`; verify is a git-diff scope check, no build gate → no deps needed). `buildReviewBrief` pure. `MAX_AUTONOMOUS_DIAGNOSTIC_ROUNDS=2`. |
